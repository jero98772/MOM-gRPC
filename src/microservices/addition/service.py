...
def fetch_queued_messages():
    ...
    while True:
        try:
            response = requests.get(f"{MOM_SERVICE}/messages/add")
...
def serve():
    ...
    server.start()
    print("Addition Service started on port 50051")
    
    # Start background thread to poll for queued messages
    threading_daemon = True
    message_thread = threading.Thread(target=fetch_queued_messages, daemon=threading_daemon)
